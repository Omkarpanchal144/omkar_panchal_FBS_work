...
for i in range(1, num_passengers + 1):
    ...
    if age < 12:
        ...
        print(f"Passenger {i} (Child): ₹{fare:.2f}")
    elif age > 59:
        fare = ticket_cost * 0.50
        print(f"Passenger {i} (Senior Citizen): ₹{fare:.2f}")
    else:
        fare = ticket_cost
        print(f"Passenger {i} (Normal): ₹{fare:.2f}")

    total_amount += fare

print(f"\nTotal amount to be paid for all passengers: ₹{total_amount}")
